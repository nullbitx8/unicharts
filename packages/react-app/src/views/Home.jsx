...
const _handleVote = ({ address, isUpvote, tx, writeContracts, messageIdx }) => {
  try {
    ...
    tx(writeContracts.NFcharT.voteOnPublicMessage(parseInt(messageIdx), isUpvote));
...
const getPublicMessages = async ({
  ...
  if (!didFetchLastPage && readContracts && readContracts.NFcharT) {
    try {
      const [nextPage, nextOffset] = await readContracts.NFcharT.getPublicMessages(limit, offset);
