...
export default function Profile({ isLoggedIn, address, userProfile, setUserProfile, tx, writeContracts }) {
    ...
    const handleCreateClick = () => {
      ...
      if (isLoggedIn) {
        try {
          ...
          tx(writeContracts.NFcharT.createUserProfileFlow(address, _name, _image1, "", "", _bio));
        } catch (e) {
          console.log(e);
        }
      }
      ...
      if (didNameChange || didBioChange || didImage1Change) {
        ...
        if (isLoggedIn) {
          try {
            ...
            tx(
              writeContracts.NFcharT.editProfile(
                address,
                didNameChange,
                _name,
                didImage1Change,
  ...
  if (!writeContracts || !writeContracts.NFcharT) return <div>Still loading</div>;
