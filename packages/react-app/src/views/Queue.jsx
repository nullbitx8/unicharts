...
const checkForMatch = async ({ readContracts, swiper, swipee, didJustMatch, setDidJustMatch }) => {
  ...
  const isMatch = await readContracts.NFcharT.getIsMatch(swiper, swipee);
  debugger;
...
const fetchProfiles = async ({
  ...
  {
    if (queue.length <= 2 && !didFetchLastPage && readContracts && readContracts.NFcharT) {
      // have at least two before fetching more
      // and if user is not logged in, then isBurner wallet is true
      try {
        const [nextPage, nextOffset] = await readContracts.NFcharT.getUnseenProfiles(
          address,
          limit,
          offset,
          !isLoggedIn,
  ...
  const swipePage = ({ isLoggedIn, writeContracts, readContracts, address, tx }) => {
    ...
    const _handleSwipe = ({ isRightSwipe, isLoggedIn, swipedProfile }) => {
      if (isLoggedIn) {
        ...
        isRightSwipe
          ? tx(writeContracts.NFcharT.swipeRight(address, swipedProfile._address))
          : tx(writeContracts.NFcharT.swipeLeft(address, swipedProfile._address));
