...
}) => {
  {
    if (!didFetchLastPage && readContracts && readContracts.NFcharT) {
      try {
        ...
        const [nextPage, nextOffset] = await readContracts.NFcharT.getRecentMessagesForMatch(
          sender,
          recipient,
          limit,
          offset,
...
export default function Messages({ isLoggedIn, sender, readContracts, writeContracts, tx }) {
  ...
  const _sendMessage = ({ messageText, isPublic }) => {
    ...
    tx(writeContracts.NFcharT.sendMessage(sender, recipient, messageText, isPublic));
