...
interface Datetime {
    function getHour(uint256 timestamp) external view returns (uint16);

    function getDay(uint256 timestamp) external view returns (uint16);
}
...
contract UniCharts is ERC721Enumerable, Ownable, ReentrancyGuard {
    // libraries
     ...
     */
    function userMint(address token0, address token1)
        public
        payable
        nonReentrant
    {
        require(!paused, "Sale paused");
        require(token0 != token1, "Same token0 and token1");
     ...
     * Transfers the minting cost to the team and the service provider.
     */
    function returnPairKey(address token0, address token1)
        public
        pure
     ...
     */
    function chartTitleSVG(string memory symbol0, string memory symbol1)
        internal
        pure
        returns (bytes memory)
    {
     ...
     */
    function yAxisSVG(string memory symbol1)
        internal
        pure
        returns (bytes memory)
    {
     ...
     */
    function buildTimeUnitsSVG(bool isWeek, uint16 xInterval)
        internal
        view
        returns (bytes memory toReturn)
    {
        ...
        returns (uint16[7] memory units)
    {
        ...
        if (isWeek) {
            for (uint256 i = 0; i < 7; i++) {
                units[i] = ethDT.getDay(
                    // solhint-disable-next-line not-rely-on-time
                    block.timestamp - (hour * 24 * (6 - i))
                );
            }
        }
     ...
     */
    function buildPricePointsSVG(uint256[] memory twips, uint16 xInterval)
        internal
        pure
        returns (bytes memory toReturn)
    {
     ...
     */
    function calcNormalizedTwaps(uint256[] memory twips)
        internal
        pure
        returns (uint256[7] memory normalizedTwaps)
    {
        ...
        for (uint8 i = 0; i < twips.length; i++) {
            ...
            normalizedTwaps[i] =
                297 -
                (((((twips[i] - min) * max) / (max - min)) * 230) / max);
        }
     ...
     */
    function firstPointSVG(uint256 twap, uint256 normalizedTwap)
        internal
        pure
        returns (bytes memory)
    {
     ...
     */
    function remainingPointsSVG(
        uint256[] memory twaps,
        uint256[7] memory normalizedTwaps,
        uint16 xInterval
    ) internal pure returns (bytes memory toReturn) {
        ...
        returns (string memory)
    {
        ...
        string memory pairName = string(
            abi.encodePacked('"', symbol0, "/", symbol1, '"') // solhint-disable-line quotes
        );
        ...
        for (uint256 i = 0; i < twipCountToFetch; i++) {
            ...
            uint256 twip = oracle.assetToAsset(
                tokens[0],
                1e18,
                tokens[1],
                currentLookbackWindow
            );
            twips[i] = twip;
        }
